  ...
  try {
    ...
    jsonLdScripts.each(function(index) {
      try {
        ...
        console.log(`\n📋 Skrypt ${index + 1}:`, typeof data, data['@type'] || 'brak @type');
        ...
        if (data['@graph']) {
          ...
          for (const item of data['@graph']) {
            if (item['@type'] === 'Product' && item.offers) {
              ...
              console.log('   📦 Offers:', typeof item.offers, Array.isArray(item.offers) ? `array[${item.offers.length}]` : 'object');
              ...
              if (Array.isArray(item.offers)) {
                ...
                console.log(`   ✅ Single Availability: ${item.offers.availability}`);
        ...
        if (data['@type'] === 'Product' && data.offers) {
          ...
          if (data.offers.availability) {
            console.log(`   ✅ Direct Availability: ${data.offers.availability}`);
        ...
        console.log(`   ❌ Błąd parsowania skryptu ${index + 1}:`, e.message.substring(0, 50));
      }
    });
    ...
    console.log(`   .stock: ${stockEl.length ? stockEl.text().trim() || 'pusty' : 'nie znaleziono'}`);
    
    const variationsForm = $('.variations_form');
    console.log(`   .variations_form: ${variationsForm.length ? 'znaleziono' : 'nie znaleziono'}`);
    
    const addToCartBtn = $('button[name="add-to-cart"], input[name="add-to-cart"], .add-to-cart-button');
    console.log(`   przycisk koszyka: ${addToCartBtn.length ? (addToCartBtn.prop('disabled') ? 'nieaktywny' : 'aktywny') : 'nie znaleziono'}`);
