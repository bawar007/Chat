...
async function testSingleProduct(url) {
  try {
    ...
    if (colorList.length > 0) {
      console.log(
        `  ✅ Znaleziono listę kolorów (${colorList.length} elementów)`
      );
    ...
    if ($(".price .woocommerce-Price-amount bdi").length > 0)
      workingSelectors.push("✅ .price .woocommerce-Price-amount bdi (cena)");
    if ($("h1").length > 0) workingSelectors.push("✅ h1 (nazwa)");
    if ($(".color-attribute-select").length > 0)
      workingSelectors.push("✅ .color-attribute-select (kolory)");
