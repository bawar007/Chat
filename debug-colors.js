...
const files = ['tabou.json', 'tabou_part10.json', 'tabou_part11.json', 'tabou_part12.json'];
      ...
      if (doc.metadata && (doc.metadata.price || doc.text.includes('zł'))) {
        ...
        if (doc.metadata.colors && doc.metadata.colors.length > 0) {
          ...
          if (colorCount === 1) {
            ...
            console.log(`     Dostępność: ${doc.metadata.availability || 'brak'}`);
          }
        }
      }
    });
...
searchTerms.forEach(term => {
  ...
  files.forEach(fileName => {
    try {
      ...
      const found = data.filter(doc => 
        doc.text && doc.text.toUpperCase().includes(term)
      );
      
      if (found.length > 0) {
        console.log(`  📄 ${fileName}: ${found.length} wyników`);
        found.forEach((doc, i) => {
          if (i < 2) { // Pokaż tylko 2 pierwsze
            console.log(`    ${i+1}. ${doc.text.slice(0, 60)}...`);
            if (doc.metadata) {
              if (doc.metadata.colors) console.log(`       Kolory: ${doc.metadata.colors.join(', ')}`);
              if (doc.metadata.availability) console.log(`       Dostępność: ${doc.metadata.availability}`);
              if (doc.metadata.price) console.log(`       Cena: ${doc.metadata.price}`);
            }
          }
        });
      }
